...
win = '<span class="btn btn-success btn-sm">WIN</span>'

def pr(string):
    string = re.sub("\s{2,}", " ", string)
    return string

def start_build_page(array):
    length = 0
    for arr in array:
        if len(arr) > length:
            length = len(arr)
    accum = []
    for arr in array:
        accum.append("<p>{:^{}}</p>".format(arr.strip(), length))
    accum.append("<br>")
    result = " ".join(accum).replace("null", "")
    return result
    ...
    if result_page:
        ...
        for additinal in additional_event:
            ...
            for bma in additinal.select(".bma-bet.sys-betting[class*=betting-won-team].betting-past"):
                ...
                if "betting-won-team1" in bma.attrs['class']:
                    temp = "<p> {} {} \t| {} \t| {} </p>".format(win,team1_bma, name_bma, team2_bma)
            
                if "betting-won-team2" in bma.attrs['class']:
                    temp = "<p> {} \t| {} \t| {} {} </p>".format(team1_bma, name_bma, team2_bma, win)
        ...
        for additional in additional_event2:
            ...
            if "betting-won-team1" in score.attrs["class"]:
                temp += "<p>{} {} \t| {} \t| {} </p>".format(win,team1score, name_score, team2score)
            
            if "betting-won-team2" in score.attrs["class"]:
                temp += "<p> {} \t| {} \t| {} {}</p>".format(team1score, name_score, team2score, win)
            build.append(temp)
            ...
            temp = ' <h3>{} \t| {} \t| {}</h3>'.format(team1score02, name_score02, team2score02)
            build.append(temp)
            ...
            for item in soup.select(".bma-bet.sys-betting.betting-past"):
                ...
                if "betting-won-team1" in item.attrs['class']:
                    temp = '<p> {} {} \t| {} \t| {} </p>'.format(win,bma_team1, bma_middle, bma_team2)
                if "betting-won-team2" in item.attrs['class']:
                    temp ="<p> {} \t| {} \t| {} {} </p>".format(bma_team1, bma_middle, bma_team2, win)
                build.append(temp)

        main_str = start_build_page(build)
    else:
        ...
        build.extend(['<h3>'+pr(datetime)+'</h3>', temp])
